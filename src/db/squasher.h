...
public:
    ...
    auto operator()(const SimpleString& s) -> void {
        sender_->SendSimpleString(s);
    }
    ...
    auto operator()(const Integer& i) -> void {
        sender_->SendInteger(i);
    }
    auto operator()(const Error& e) -> void {
        sender_->SendError(e);
    }
    auto operator()(const Null&) -> void {
        sender_->SendNullBulkString();
    }
private:
    Sender* sender_;
};
...
public:
    ...
    auto SendOk() -> void override {
        payload_ = SimpleString("OK");
    }
    auto SendPong() -> void override {
        payload_ = SimpleString("PONG");
    }
    ...
    auto SendInteger(int64_t value) -> void override {
        payload_ = Integer(value);
    }
    auto SendError(std::string_view s) -> void override {
        payload_ = Error(std::string(s));
    }
...
public:
    ...
    static auto Squash(std::vector<CommandContext>& cmds) -> void;
